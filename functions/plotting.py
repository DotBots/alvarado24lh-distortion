...
def plot_transformed_3D_data(df):
    ...
    stop_idx = 40
    stop_idx = df.shape[0]-1
    color=np.arange(stop_idx) #+339

    # Plot the ground truth points
    real_points = df[['real_x_mm','real_y_mm','real_z_mm']].to_numpy()[0:stop_idx]  # Plot just a single point per grid position to save on computational power.
    # ax2.scatter(real_points[:,0], real_points[:,1], real_points[:,2], alpha=0.5 ,color='xkcd:green', label="ground truth")
    ax2.scatter(real_points[:,0], real_points[:,1], real_points[:,2], alpha=0.5 ,c=color, cmap='winter', label="ground truth")
    ...
    points = df[['Rt_x','Rt_y','Rt_z']].to_numpy()[0:stop_idx]
    ax2.scatter(points[:,0], points[:,1], points[:,2], alpha=0.8, c=color, cmap='autumn', label="lighthouse")
    # r_points = df[['LH_x','LH_y','LH_z']].to_numpy()
    # ax2.scatter(r_points[:,0], r_points[:,1], r_points[:,2], alpha=0.8, color='xkcd:red')
    ax2.axis('equal')
    ax2.legend()
    ...
    # Set the view rotation for the plot
...
color_list = [['xkcd:blue', 'xkcd:indigo'],
              ['xkcd:light red', 'xkcd:pastel pink'],
              ['xkcd:jungle green', 'xkcd:pastel green'],
              ['xkcd:pinkish purple', 'xkcd:light lavender'],
              ['xkcd:orange', 'xkcd:light tan'],
              ['xkcd:midnight', 'xkcd:pale grey'],
              ]

def plot_acc_vs_npoints_noise(df_plot):
    
    # Find out how many unique N_Points are available in this experiment
    unique_noise = np.unique(df_plot['noise'].to_numpy(), axis=0)
    unique_points = np.unique(df_plot['n_points'].to_numpy(), axis=0)
    ...
    for color_id, n_point in enumerate(unique_points):
        # Go through all the available N_point experiments
        mae_std = np.empty((unique_noise.shape[0],3))
        for idx, i in enumerate(unique_noise):
            # Get the mean of the MAE, and the STD of the MAE
            mae = df_plot.loc[(df_plot['noise'] == i) & (df_plot['n_points'] == n_point), 'MAE'].values.mean(axis=0)    
            std = df_plot.loc[(df_plot['noise'] == i) & (df_plot['n_points'] == n_point), 'MAE'].values.std(axis=0)
            # Add it to our empty array for plotting later
            mae_std[idx] = np.array([i, mae, std])    

        # Plot Y = MAE, X = N_points
        error_ax.plot(mae_std[:,0], mae_std[:,1], color=color_list[color_id][0])
        error_ax.scatter(mae_std[:,0], mae_std[:,1], color=color_list[color_id][0], label=f"{n_point} points")
        # Add and area with 2 std deviation
        error_ax.fill_between(mae_std[:,0], np.clip(mae_std[:,1] - 1*mae_std[:,2], 0.0, 1e10), mae_std[:,1] + 1*mae_std[:,2], alpha=0.2, edgecolor=color_list[color_id][1], linestyle='dashed', antialiased=True)
        # error_ax.fill_between(mae_std[:,0], np.clip(mae_std[:,1] - 1*mae_std[:,2], 0.0, 1e10), mae_std[:,1] + 1*mae_std[:,2], alpha=0.2, edgecolor=color_list[color_id][1], facecolor='lightblue', linestyle='dashed', antialiased=True)
    ...
    error_ax.set_xlabel('Injected noise [deg]')
    error_ax.set_ylabel('Mean Average Error [mm]')
